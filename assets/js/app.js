...
async function aktualisiere(){
  ...
  try{
    ...
    if('caches' in window){ try{ const c=await caches.open('ki-zeitung-v2-20260902'); await c.delete('data/mock.json'); await c.delete('data/mock.json'+bust); }catch(e){} }
    const [s,d]=await Promise.all([
    ...
    SOURCES=s; DATA=d;
    const ds=DATA.tageslage.date.split('-'); const dstr=ds[2]+'.'+ds[1]+'.'+ds[0];
    if(pill) pill.textContent='● Tageslage '+dstr+' 06:00 MEZ ✓';
    ...
    n.textContent='✓ Aktualisiert — '+dstr+' '+DATA.tageslage.schluss; document.body.appendChild(n); setTimeout(()=>n.remove(),2200);
    setTimeout(()=>{ if(pill) pill.textContent='● Tageslage '+dstr+' 06:00 MEZ'; },2500);
  }catch(e){
...
async function testCloudLLM(){
  ...
  if(!key){ s.textContent='✗ Kein Key — bitte oben einfügen (Groq kostenlos: console.groq.com/keys)'; s.style.color='#ff6b6b'; return; }
...
function searchTopic(){
  ...
  if(synthHits.length===0 && perspHits.length===0 && zahlenHits.length===0){
    ...
    html+='1) Cloud-Key oben speichern → 2) Thema unten neu synthetisieren lassen → 3) Als neue Tageslage exportieren. Oder RSS live via CH-Proxy durchsuchen: <code>backend/ch-proxy.js</code> + LLM-Prompt mit "'+q.replace(/</g,'&lt;')+'".</div>';
...
async function searchTopicWithLLM(){
  ...
  const resEl=$('#search-results');
  if(!key){ resEl.innerHTML+='<div class=notice style="border-color:#ff6b6b">✗ Für Tiefen-Recherche API-Key oben im hellgelben Feld speichern (openrouter.ai/keys → sk-or-...). Mock-Suche bleibt ohne Key.</div>'; return; }
